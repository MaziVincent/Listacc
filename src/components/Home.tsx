    ...
    <div className="font-Sora">
        ...
        <div className="bg-gradient-to-tr from-textColorOne to-slate-900 py-[20px] h-screen flex flex-col justify-evenly">
            <div className="overflow-hidden flex w-full " >
                {
                    slides.map((slide)=>(
                        <div key={slide.id} className="w-full h-full flex items-center xs:justify-center lg:justify-start flex-shrink-0 transition-transform duration-[5s] ease-in-out m-auto" style={{ transform: `translateX(-${currentSlide * 100}%)` }}>
                            {slide.content}
                        </div>
                    ))
                }
            ...
            <div className="flex justify-evenly flex-row m-[20px] w-[30%] mx-auto ">
                {
                    slides.map((slide)=>(
                        <button key={slide.id} onClick={()=> goToSlide(slide.id)} className={`border-[1px] transform-border ease-out duration-[5s] rounded-full p-[5px] flex items-center justify-center ${currentSlide === slide.id ? "border-[#c7c7c7] text-white": "border-none text-[#c7c7c7]" }`} >
                            <GoDotFill />
                        </button>
                    ))
                }
        ...
        <div className="py-[40px] flex flex-wrap border-b-[1px] border-[#f5f5f5] ">
            <p className="flex text-center justify-center p-[20px] space-x-3 xs:w-full md:w-[50%] lg:w-[25%]"><span><GoShieldCheck  className="text-3xl text-textColorOne "/> </span>World-class services</p>
            <p className="flex text-center justify-center p-[20px] space-x-3 xs:w-full md:w-[50%] lg:w-[25%]"><span><BsHourglass  className="text-3xl text-textColorOne "/></span>Experience strategy</p>
            <p className="flex text-center justify-center p-[20px] space-x-3 xs:w-full md:w-[50%] lg:w-[25%]"><span><BsAward  className="text-3xl text-textColorOne "/></span>Award winning agency</p>
            <p className="flex text-center justify-center p-[20px] space-x-3 xs:w-full md:w-[50%] lg:w-[25%]"><span><PiSuitcaseSimpleThin className="text-3xl text-textColorOne "/></span>Grow your business</p>
        </div> 
        {/* end second section */}
        {/* Start section two */}
        <div className="xs:text-center lg:text-left py-[50px] xs:w-[85%] lg:w-[30%] xs:mx-auto lg:mx-[50px] space-y-4">
            <button className="py-[10px] px-[20px] text-textColorOne bg-sectionthreecolor rounded-full text-[13px] font-bold">ABOUT BUSINESS</button>
            <p className="xs:text-[30px] lg:text-[44px] font-bold">Smart and effective business solutions.</p>
            <p className="text-[#868D98]">We are excited for our work and how it positively impacts clients. With over 12 years of experience we have been constantly providing excellent solutions.</p>
            <div className="flex items-center space-x-2">
                <div className="border-[5px] p-[30px] border-textColorOne rounded-full w-[]">
                    90%
                </div>
                <p className="text-[20px]">Increased revenue in the <span className="text-textColorOne font-bold underline">last 2 years.</span></p>
            </div>
        </div>
        {/* end section 2 */}
        {/* start section 3 */}
        ...
        <div className="py-[50px] px-[15px] flex xs:flex-col lg:flex-row w-[80%] mx-auto">
            <div className="xs:text-center lg:text-left space-y-[25px] xs:w-full lg:w-[50%]">
                <p className="bg-sectionthreecolor text-textColorOne w-[172px] m-auto text-[13px] py-[15px] px-[10px] rounded-full font-bold text-center">FLEXIBLE PRICING</p>
                <h3 className="text-dropdown text-[33.37px] w-[90%] font-bold">Tailored pricing plans for everyone.</h3>
                <p className="text-textColorTwo text-[17px]">We are excited for our work and how it positively impacts clients. With over 12 years of experience we have been constantly providing excellent solutions.</p>
                <button className="bg-dropdown text-white w-[60%] m-auto text-[13px] py-[15px] px-[10px] rounded-full font-bold">VIEW ALL PLANS</button>
            </div>
            <div className="xs:w-full lg:w-[50%] my-[50px] max-w-md mx-auto space-y-5">
      {planData.map((plan, index) => (
        <div key={index} className={`border rounded-md overflow-hidden shadow ${openIndex === index ? "bg-textColorOne text-white" : ""}`}>
          {/* Header */}
          <div
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold">
                ${plan.price} <span className="text-sm font-normal">/ Monthly</span>
              </h3>
